...
function SelectItem({
 ...
 className={({ isHovered, isFocused, isSelected }) =>
  cn(
    "relative flex w-full items-center gap-2.5 py-2 pr-8 pl-3 text-sm",
   isSelected
...
>
      {composeRenderProps(children, (children, { isSelected }) => (
        ...
        <span className="pointer-events-none absolute right-2 flex size-4 items-center justify-center text-inherit">
...
function SelectSeparator({
  ...
  return (
    <SeparatorPrimitive
      ...
      className={cn(
        "pointer-events-none -mx-1.5 my-1.5 h-px bg-zinc-200 dark:bg-zinc-700",
        className
      )}
      {...props}
    />
...
function SelectEmpty({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn(
        "hidden w-full justify-center py-2 text-center text-sm text-muted-foreground dark:text-zinc-400 group-data-empty/select-list:flex",
        className
      )}
      {...props}
    />
